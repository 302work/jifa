...
public class RedisKit {
    ...
    static void init(ShardedJedisPool shardedJedisPool) {
        RedisKit.shardedJedisPool = shardedJedisPool;
    }

    static ShardedJedisPool  getShardedJedisPool(){
        return shardedJedisPool;
    }

    /**
     * 执行器，{@link com.jfinal.ext.plugin.redis.RedisKit}的辅助类，
     ...
     * @return 返回生成的Id
     */
    public static long makeId(final String key) {
        return new Executor<Long>(shardedJedisPool) {
     ...
     */
    public static String setString(final String key, final String value, final int expire) {
        return new Executor<String>(shardedJedisPool) {
     ...
     * @return 操作状态的集合
     */
    public static List<Object> batchSetString(final List<Pair<String, String>> pairs) {
     ...
     * @return value的集合
     */
    public static List<String> batchGetString(final String[] keys) {
    ...
    /* ======================================Object====================================== */
    /**
     * 将对象obj转成json后 关联到 key 。
     * 如果 key 已经持有其他值， set 就覆写旧值，无视类型。
     * 对于某个原本带有生存时间（TTL）的键来说， 当 set 成功在这个键上执行时， 这个键原有的 TTL 将被清除。
     * 时间复杂度：O(1)
     * @param key key
     * @param obj Object
     * @return 在设置操作成功完成时，才返回 OK 。
    ...
    public static String set(String key, Object obj) {
        return RedisKit.setString(key, JSON.toJSONString(obj));
    }

    /**
     * 将值 obj 关联到 key ，并将 key 的生存时间设为 expire (以秒为单位)。
     * 如果 key 已经存在， 将覆写旧值。
     * 类似于以下两个命令:
     * SET key value
     * EXPIRE key expire # 设置生存时间
     ...
     * @return 设置成功时返回 OK 。当 expire 参数不合法时，返回一个错误。
     */
    public static String set(String key, Object obj, int expire) {
        return RedisKit.setString(key, JSON.toJSONString(obj), expire);
    }
     ...
     * @return 设置成功，返回 1 。设置失败，返回 0 。
     */
    public static Long setIfNotExists(String key, Object obj) {
        return RedisKit.setStringIfNotExists(key, JSON.toJSONString(obj));
    }
    /**
     * 根据key 获取对象
     * @param key
     * @param clazz 非Model的实体类
     * @return
     */
    public static <T> T get(String key,Class<T> clazz){
        String value = RedisKit.getString(key);
     ...
     * 如果存放的是多个model取第一个的值
     * @param key
     * @param clazz Model的实体类
     * @return
     */
    public static <T extends Model<T>> T getModel(String key,Class<T> clazz){
        String value = RedisKit.getString(key);
        if(StringUtils.isEmpty(value)){
            return null;
        }
        try {
            ...
            if(value.indexOf("[")==0){
                jsonObj = JSON.parseArray(value).getJSONObject(0);
            }else{
                jsonObj = JSON.parseObject(value);
            }
            return buildModel(jsonObj, clazz);
        } catch (Exception e) {
            e.printStackTrace();
        }
        return null;
    }
//    [{"attrNames":["id","userName"],"attrValues":[1,"111"],
//    	"attrsEntrySet":[{"key":"id","value":1},{"key":"userName","value":"111"}]},{"attrNames":["id","createTime
//    	","admin","userName"],"attrValues":[2,1427188406459,false,"222"],"attrsEntrySet":[{"key":"id","value":2},
//    	{"key":"createTime","value":1427188406459},{"key":"admi
//    	n","value":false},{"key":"userName","value":"222"}]}]
    /**
     * 根据key 获取多个Model的实体类对象
     * @param key
     * @param clazz
     * @return
     */
    public static <T extends Model<T>> List<T> getModels(String key,Class<T> clazz){
        String value = RedisKit.getString(key);
        if(StringUtils.isEmpty(value)){
            return null;
        }
        try {
            ...
            if(value.indexOf("[")==0){
                ...
                for (int i = 0; i < jsonArray.size(); i++) {
                    list.add(buildModel(jsonArray.getJSONObject(i), clazz));
                }
            }else{
                jsonObj = JSON.parseObject(value);
                list.add(buildModel(jsonObj, clazz));
            }
            return list;
        } catch (Exception e) {
            e.printStackTrace();
     ...
     * @throws Exception
     */
    private static  <T extends Model<T>> T buildModel(JSONObject jsonObj,Class<T> clazz) throws Exception{
        if(jsonObj==null || !jsonObj.containsKey("attrsEntrySet")){
            return null;
        }
        JSONArray jsonArray = jsonObj.getJSONArray("attrsEntrySet");
        if(jsonArray!=null && jsonArray.size()>0){
            T t = clazz.newInstance();
            for (int i=0;i<jsonArray.size();i++) {
                String skey = jsonArray.getJSONObject(i).getString("key");
                String svalue = jsonArray.getJSONObject(i).getString("value");
                //获取当前字段的类型
                String colClassName = TableMapping.me().getTable(clazz).getColumnTypeMap().get(skey).getName();
                t.set(skey, getObjectValue(svalue,colClassName));
            }
            return t;
        }
        return null;
     ...
     * @return
     */
    private static Object getObjectValue(String svalue,String colClassName){
        //如果是日期类型，将毫秒转换为日期类型
        if ("java.sql.Date".equals(colClassName) || "java.sql.Timestamp".equals(colClassName) || "java.sql.Time".equals(colClassName)) {
            return DateUtil.getDate(Long.valueOf(svalue));
        }
        else if ("java.lang.String".equals(colClassName)) {
            return svalue;
        }
     ...
     */
    public static Long listPushHead(final String key, final String value) {
        return new Executor<Long>(shardedJedisPool) {
     ...
     */
    public static Long listPushHeadAndTrim(final String key, final String value, final long size) {
        return new Executor<Long>(shardedJedisPool) {
